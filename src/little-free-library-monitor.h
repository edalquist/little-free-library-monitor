...
static const std::chrono::duration<int, std::milli> EVENT_RATE_LIMIT_MILLIS =
    250ms;
static const std::chrono::duration<int> SLEEP_DELAY =
    5s;  // How long to wait after an event to sleep
static const int SHORT_SLEEP_SOC_THRESHOLD = 70;  // at 70% start increasing
static const std::chrono::duration<int> SHORT_SLEEP_DURATION =
    5min;  // How long to sleep when SOC is above SHORT_SLEEP_SOC_THRESHOLD
static const std::chrono::duration<int> LONG_SLEEP_DURATION =
    60min;  // How long to sleep
static const std::chrono::duration<int> CONNECT_INTERVAL =
    60s;  // How long to wait before ensuring MQTT server is connected
